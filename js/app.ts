const app = {
    ...
    syncBasicDefaults: function() {
        const product = (document.getElementById('basic-product') as HTMLSelectElement)?.value || 'rice';
        const state = (document.getElementById('basic-state') as HTMLSelectElement)?.value || 'dry';
        const shelf = parseInt((document.getElementById('basic-shelf') as HTMLSelectElement)?.value || '30', 10);
        const storage = (document.getElementById('basic-storage') as HTMLSelectElement)?.value || 'room';
        const transport = (document.getElementById('basic-transport') as HTMLSelectElement)?.value || 'local';
    ...
    applyTranslations: function() {
        ...
        if (modeBtn) modeBtn.innerText = this.mode === 'simple' ? (this.lang === 'hi' ? 'सरल मोड' : 'Basic Mode') : (this.lang === 'hi' ? 'विशेषज्ञ मोड' : 'Expert Mode');
    ...
    runAnalysis: async function(e) {
        ...
        this.currentCommodity = {
            ...
            priority: (document.getElementById('basic-priority') as HTMLSelectElement)?.value || 'balanced',
            transportationCondition: (document.getElementById('basic-transport') as HTMLSelectElement)?.selectedOptions[0]?.text || 'Not specified'
        };
    ...
    renderResults: function() {
        ...
        const simplifyReason = (reason) => {
            if (reason.includes('temperature')) return 'Storage temperature is outside this material range.';
            if (reason.includes('moisture') || reason.includes('WVTR')) return 'Insufficient moisture protection for this product.';
            if (reason.includes('fat') || reason.includes('grease')) return 'Not enough grease resistance for this product.';
            if (reason.includes('frozen') || reason.includes('low-temperature')) return 'Not flexible enough for frozen storage.';
            if (reason.includes('OTR') || reason.includes('respiration')) return 'Gas permeability does not match respiration demand.';
            if (reason.includes('budget') || reason.includes('cost')) return 'Higher cost than the available budget.';
