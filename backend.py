...
def trace_page(record):
    ...
    package_area = float(geometry.get("surfaceAreaM2") or commodity.get("pkgArea") or 0.05)
    material_cost_per_pack = float(material.get("costPerM2", 0)) * package_area
    storage_condition = "Frozen" if commodity.get("temp", 25) < 0 else "Chilled" if commodity.get("temp", 25) <= 8 else "Ambient"
    transport_condition = commodity.get("transportationCondition", "Not specified")
    package_format = record.get("packageFormat", "Flexible film or pouch")
    thickness = material.get("testConditions", {}).get("otr", {}).get("thicknessUm") or material.get("testConditions", {}).get("wvtr", {}).get("thicknessUm") or "Screening range: 30–50"
    map_applicable = bool(map_result.get("applicable"))
    map_suitability = "Suitable" if map_applicable else "Not applicable"
    why_packsmart = "This packaging provides suitable moisture and oxygen protection for the product while meeting the selected cost and sustainability priorities."
...
<div class="hero"><div class="eyebrow">PackSmart AI traceability report</div><h1>{escape(commodity["name"])}</h1><p>Traceability and packaging report for batch <strong>{escape(record["batchId"])}</strong>.</p><p class="trust">✓ Screening recommendation · Validate before commercial use</p></div>
<section><h2>Product information</h2><div class="grid"><div class="fact"><div class="label">Product / commodity</div><div class="value">{escape(commodity["name"])}</div></div><div class="fact"><div class="label">Batch / lot ID</div><div class="value">{escape(record["batchId"])}</div></div><div class="fact"><div class="label">Packaging date</div><div class="value">{escape(shelf["packDate"])}</div></div><div class="fact"><div class="label">Recommended shelf life</div><div class="value">{shelf["predictedDays"]} days</div></div><div class="fact"><div class="label">Storage condition</div><div class="value">{storage_condition} · {commodity["temp"]}°C</div></div><div class="fact"><div class="label">Transportation condition</div><div class="value">{escape(str(transport_condition))}</div></div></div></section>
<section><h2>Packaging recommendation</h2><div class="grid"><div class="fact"><div class="label">Recommended material</div><div class="value">{escape(top["name"])}</div></div><div class="fact"><div class="label">Packaging format</div><div class="value">{escape(str(package_format))}</div></div><div class="fact"><div class="label">Suggested thickness</div><div class="value">{escape(str(thickness))} µm</div></div><div class="fact"><div class="label">OTR requirement</div><div class="value">{material["otr"]} cc/m²/day</div></div><div class="fact"><div class="label">WVTR requirement</div><div class="value">{material["wvtr"]} g/m²/day</div></div><div class="fact"><div class="label">Sealability</div><div class="value">Suitable</div></div><div class="fact"><div class="label">Mechanical strength</div><div class="value">{'High' if commodity.get('category') in {'meat', 'grain'} else 'Medium'}</div></div><div class="fact"><div class="label">MAP suitability</div><div class="value">{map_suitability}</div></div></div><p>Screening material cost: ₹{material_cost_per_pack:.2f} per pack. Material-only estimate; conversion and manufacturing costs are not included.</p></section>
<section><h2>Why PackSmart selected this</h2><p>{why_packsmart}</p><p class="trust">Recyclability: {'Yes' if material["recyclable"] else 'No'} · Sustainability screening: {sustainability["total"]}/100</p></section>
<section><button id="technical-toggle" type="button">View technical specs</button></section>
<section id="technical"><h2>Technical record</h2><div class="grid"><div class="fact"><div class="label">Recommendation and review</div><p>Recommendation ID: {escape(record["analysisId"])}<br>Review status: {escape(record.get("reviewStatus", "Preliminary screening"))}<br>Confidence: {escape(str(confidence.get("level", "Screening")))} ({float(confidence.get("score", 0)) * 100:.1f}%)<br>Uncertainty: {escape(str(confidence.get("uncertainty", "")))}</p></div><div class="fact"><div class="label">Original inputs</div><pre>{escape(json.dumps(commodity, indent=2, ensure_ascii=False))}</pre></div><div class="fact"><div class="label">Package format and geometry</div><p>Format: {escape(str(record.get("packageFormat", "Flexible film or pouch")))}<br>Surface area: {geometry.get("surfaceAreaM2", "-")} m²<br>Package weight: {geometry.get("packageWeightKg", "-")} kg<br>Recommended structure: {escape(material.get("name", top.get("name", "Unknown")))}</p></div><div class="fact"><div class="label">Material specification and test conditions</div><p>OTR: {material.get("otr", "-")} cc/m²/day<br>WVTR: {material.get("wvtr", "-")} g/m²/day<br>Cost: ₹{material.get("costPerM2", "-")}/m²<br>Grease resistance: {material.get("greaseResistance", "Not recorded")}<br>Low-temperature flexibility: {material.get("lowTempFlex", "Not recorded")}<br>Test temperature: {test_conditions.get("temperatureC", commodity["temp"])}°C<br>Test RH: {test_conditions.get("relativeHumidityPercent", commodity["rh"])}%</p></div><div class="fact"><div class="label">Sustainability breakdown</div><p>Recyclability: {sustainability["recyclability"]}<br>Biodegradability: {sustainability["biodegradability"]}<br>Carbon impact: {sustainability["carbon"]}<br>Total: {sustainability["total"]}/100</p></div><div class="fact"><div class="label">MAP calculation</div><pre>{escape(json.dumps(map_result.get("specs", {}), indent=2, ensure_ascii=False))}</pre></div></div><h3>Laboratory validation checklist</h3><ul>{''.join(f'<li>{escape(item)}</li>' for item in record.get("laboratoryValidationChecklist", []))}</ul><h3>All material evaluations</h3><div class="table-wrap"><table><thead><tr><th>Material</th><th>Status</th><th>Barrier</th><th>Cost</th><th>Sustainability</th><th>TOPSIS</th><th>Elimination reason</th></tr></thead><tbody>{technical_rows}</tbody></table></div><p class="label">Computed {escape(record.get("timestamp", "Not recorded"))} · Engine {escape(record.get("engineVersion", "unknown"))} · Database {escape(record.get("databaseVersion", "unknown"))}</p><p><strong>Preliminary screening result. Not a production release or shelf-life certificate.</strong></p></section>
