const Engine = {
    ...
    runTOPSIS: function (commodity, candidates, priority = 'balanced') {
        ...
        let matrix = candidates.map(mat => {
            ...
            let mechVal = 50;
            if (commodity.category === 'dairy' && mat.id === 'pet')
                mechVal = 100;
            if (commodity.category === 'grain' && (mat.id === 'bopp' || mat.id === 'ldpe'))
                mechVal = 100;
            if (commodity.category === 'meat' && mat.id === 'nylon')
                mechVal = 100;
    ...
    generateExplanation: function (topMat, commodity, rejections, lang = 'en') {
        ...
        if (isHi) {
            ...
            if (commodity.temp < 0)
                text += `इसमें निम्न-तापमान लचीलापन (फ्रोजन भंडारण के लिए आवश्यक) है। `;
            if (commodity.fat > 5)
                text += `यह वसायुक्त खाद्य पदार्थों के लिए आवश्यक ग्रीस प्रतिरोध प्रदान करता है। `;
        }
        else {
            text += `<strong>${topMat.name}</strong> was selected because it perfectly passes the physical constraints of your product. `;
            if (commodity.temp < 0)
                text += `It has excellent low-temperature flexibility required for frozen storage. `;
            if (commodity.fat > 5)
                text += `It provides the necessary grease resistance for high-fat foods. `;
        }
